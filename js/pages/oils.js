...
const loadOilsServices = async () => {
    const data = await loadData();
    cachedServices = data.oils || [];
    return cachedServices;
};

export const renderOilsPage = (store) => {
    const container = document.createElement('div');
    container.className = 'container';
    
    container.innerHTML = `
        <h1 style="margin-bottom: 2rem;">🛢️ Замена масел и техническое обслуживание</h1>
        ...
        <div class="grid-4" id="servicesGrid"></div>
    `;
    ...
    const renderServices = async () => {
        const services = await loadOilsServices();
        if (!grid) return;
        grid.innerHTML = '';
